...
usage() {
  cat <<'EOF'
Usage:
  install-bridge-from-github.sh [--pack feishu] --feishu-app-id <app_id> --feishu-app-secret <app_secret> [--codex-bin <path>] [--project-scan-roots <path1:path2:...>] [--ref <name>] [--ref-type branch|tag]
EOF
}
...
if [[ -z "$FEISHU_APP_ID" || -z "$FEISHU_APP_SECRET" ]]; then
  echo "missing --feishu-app-id or --feishu-app-secret" >&2
  usage >&2
  exit 1
fi
...
INSTALL_CMD=(node dist/cli.js install --pack "$PACK" --pack-option "app-id=$FEISHU_APP_ID" --pack-option "app-secret=$FEISHU_APP_SECRET")
if [[ -n "$CODEX_BIN" ]]; then
