...
$WorkDir = Join-Path ([System.IO.Path]::GetTempPath()) ("ctb-feishu-linker-install-" + [System.Guid]::NewGuid().ToString("N"))
...
try {
  ...
  try {
    ...
    $installArgs = @(
      "dist/cli.js", "install", "--pack", $Pack,
      "--pack-option", "app-id=$FeishuAppId",
      "--pack-option", "app-secret=$FeishuAppSecret"
    )
    if ($CodexBin) {
