...
const TELEGRAM_COMMAND_ENTRIES: LocalizedTelegramCommandEntry[] = [
  ...
  { command: "upload", handler: "handleUpload", description: { zh: "上传文件到项目根目录", en: "Upload a file to the project root" }, helpLines: [{ zh: "/upload 安全保存下一份文件到项目根目录；不会发送给 Codex", en: "/upload Safely save the next file to the project root without sending it to Codex" }], telegramOnly: true },
...
export function buildHelpText(language: UiLanguage = "zh", activePack: BridgePackName = "telegram"): string {
  const heading = language === "en" ? "Available commands" : "可用指令";
  const lines = TELEGRAM_COMMAND_ENTRIES
    .filter((entry) => !entry.telegramOnly || activePack === "telegram")
    .flatMap(({ helpLines }) => helpLines.map((line) => line[language]));
